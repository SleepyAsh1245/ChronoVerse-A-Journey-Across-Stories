...
const themes = {
  ...
  bubblegum: {'--bg': '#fff0f6','--card': '#ffd6e8','--text': '#4a003f','--muted': '#cc7da1','--accent': '#ff5fa2'}
};
...
document.getElementById('cardsBtn').addEventListener('click', ()=>{
  cancelTyping(); cancelSpeech();
  storyEl.textContent=''; tarotEl.innerHTML=''; tarotEl.classList.remove('hidden'); chapterEnd.classList.add('hidden'); 
  ...
  (chapters[current].tarot||[]).forEach(card=>{
    ...
    el.addEventListener('click', ()=>{
      wrapper.querySelectorAll('.tarot-card').forEach(c=>{ if(c!==el) c.classList.remove('flipped'); });
  ...
  showChoices([{ text:'Return', action:()=>{ tarotEl.classList.add('hidden'); chapterEnd.classList.remove('hidden'); concludeChapter(); } }]);
});
